#!/usr/bin/env python3
"""
Test with longer message to verify chunking and combination with robust fix
"""
...
async def test_specific_looping_fix():
    """Test with longer message to verify chunking and combination"""
    print("🔧 Specific Looping Fix Test - Longer Message")
    print("=" * 60)
    
    # Longer test message that should generate ~16+ seconds of audio
    test_message = "In the depths of consciousness, whispering secrets of the universe to those who dare to listen with open hearts. This message should generate approximately sixteen seconds of audio content to test the chunking and combination logic with the new robust synchronization system that prevents any playback issues including looping problems."
        ...
        if response.status_code == 200:
            ...
            if os.path.exists(local_path):
                ...
                if result.returncode == 0:
                    ...
                    # Check if this was a combined video (look for "combined" in filename)
                    if "combined" in filename:
                        print(f"🔗 This was a combined video (chunked processing)")
                    else:
                        print(f"🎬 This was a single video (no chunking)")
...
if __name__ == "__main__":
    asyncio.run(test_specific_looping_fix()) 